  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-6 duration-500 space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-5 bg-white border border-slate-200 rounded-4xl shadow-xl shadow-slate-200/40 p-8 space-y-8">
              <div className="flex items-center gap-4 border-b border-slate-100 pb-6">
                <div className="size-12 bg-blue-50 text-blue-600 rounded-[1.25rem] flex items-center justify-center shadow-inner">
                  <Network className="size-6" />
                </div>
                <div>
                  <h3 className="text-lg font-black text-slate-900 tracking-tight uppercase">{t.net_lab}</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t.intel_engine}</p>
                </div>
              </div>
              <div className="space-y-6">
                  <div className="form-control">
                    <label className="label px-1 pt-0">
                      <span className="label-text text-[11px] font-black uppercase text-slate-600 tracking-wider">{t.ipv6_block}</span>
                    </label>
                    <input 
                      type="text" 
                      value={state.ipv6Input} 
                      onChange={(e) => state.setIpv6Input(e.target.value)} 
                      className="input input-lg bg-slate-50 border-slate-200 w-full rounded-2xl text-base font-mono font-bold h-14 focus:ring-4 focus:ring-blue-100 focus:bg-white transition-all text-center shadow-sm" 
          ...
          <div className="lg:col-span-7 space-y-6">
              <div className="bg-white border border-slate-200 rounded-4xl shadow-xl shadow-slate-200/40 overflow-hidden min-h-75 flex flex-col">
                  <div className="px-8 py-5 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <span className="text-[9px] font-black bg-slate-200 text-slate-600 px-2 py-0.5 rounded uppercase">{state.labResults.length} Result{state.labResults.length !== 1 ? 's' : ''}</span>
                  </div>
                  <div className="flex-1 p-6 custom-scrollbar-light overflow-y-auto max-h-125">
                      {state.labResults.length > 0 ? (
                              ...
                              <div className="px-4 py-2 bg-slate-900 border-b border-slate-800 flex items-center justify-between">
                                ...
                                <div className="flex gap-2">
                                  <button onClick={() => copy(res.ip)} className="flex items-center gap-1.5 text-[8px] font-black text-indigo-400 hover:text-indigo-300 transition-colors uppercase">
                                    <Hash className="size-2.5" /> {t.copy_ip}
                                  </button>
                                  <button onClick={() => copy(res.arpa)} className="flex items-center gap-1.5 text-[8px] font-black text-emerald-400 hover:text-emerald-300 transition-colors uppercase">
                                    <Globe className="size-2.5" /> {t.copy_arpa}
                                  </button>
                                </div>
                              </div>
                        ...
                        <div className="h-full flex flex-col items-center justify-center py-20 opacity-20 grayscale">
                          ...
                          <p className="text-[12px] font-black uppercase tracking-[0.3em]">{t.system_idle}</p>
                        </div>
                      )}
                  </div>
              </div>
