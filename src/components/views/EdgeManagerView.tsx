...
import { 
  Globe, ChevronRight, Loader2, Edit3, Trash2, Check, X, Award, Shield, 
  CloudCog, Search, CheckSquare, Square, Zap, ZapOff 
} from 'lucide-react';
...
export const EdgeManagerView = ({ state }: { state: any }) => {
  ...
  const filteredDns = state.dnsRecords.filter((r: any) => 
        ...
        <div className="p-5 space-y-5">
           {state.selectedAccountId && (
              <div className="flex gap-2">
                <input type="text" placeholder={t.add_domain} value={state.newDomainName} onChange={(e) => state.setNewDomainName(e.target.value)} className="input input-sm bg-white border-slate-200 flex-1 rounded-xl text-[11px] font-bold h-11 focus:ring-4 focus:ring-indigo-50 shadow-sm" />
           ...
           <div className="max-h-120 overflow-y-auto space-y-2 custom-scrollbar-light px-1 pr-2 py-2">
              {!state.selectedAccountId ? (
                ...
                state.zones.map((z: any) => (
                  ...
                  >
                    <span className="text-[12px] font-black truncate flex-1 pr-4 min-w-0">{z.name}</span>
        ...
        {state.zoneId ? (
          <>
            <div className="bg-white border border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-8 space-y-6 overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-50 pb-4 min-w-0 gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-6 gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-inner">
                 ...
                 <input type="text" placeholder="Name" value={state.newDns.name} onChange={e => state.setNewDns({...state.newDns, name: e.target.value})} className="input input-sm bg-white border-slate-200 h-11 text-[11px] font-black sm:col-span-1 rounded-xl focus:ring-4 focus:ring-indigo-100 shadow-sm" />
                 <input type="text" placeholder="Content" value={state.newDns.content} onChange={e => state.setNewDns({...state.newDns, content: e.target.value})} className="input input-sm bg-white border-slate-200 h-11 text-[11px] font-black sm:col-span-2 rounded-xl focus:ring-4 focus:ring-indigo-100 shadow-sm" />
              ...
              {state.selectedDnsIds.size > 0 && (
                <div className="flex items-center justify-between bg-indigo-600 p-3 rounded-2xl shadow-lg animate-in slide-in-from-top-2 duration-300">
                  <div className="flex items-center gap-3 ml-2">
                    <CheckSquare className="size-4 text-white" />
                    <span className="text-[10px] font-black text-white uppercase tracking-widest">
                      {t.selected_count.replace('{n}', state.selectedDnsIds.size.toString())}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className="btn btn-xs bg-white/20 hover:bg-white/30 text-white border-none rounded-lg font-black uppercase text-[9px] h-8 px-3"
                    >
                      <Zap className="size-3 mr-1.5" /> {t.bulk_proxy}
                    </button>
                    <button 
                      ...
                      className="btn btn-xs bg-white/20 hover:bg-white/30 text-white border-none rounded-lg font-black uppercase text-[9px] h-8 px-3"
                    >
                      <ZapOff className="size-3 mr-1.5" /> {t.bulk_unproxy}
                    </button>
                    <button 
                      onClick={() => state.setShowBulkDeleteConfirm(true)} 
                      disabled={state.loadStates.bulk}
                      className="btn btn-xs bg-rose-500 hover:bg-rose-600 text-white border-none rounded-lg font-black uppercase text-[9px] h-8 px-3 shadow-md shadow-rose-900/20"
                    >
                      <Trash2 className="size-3 mr-1.5" /> {t.bulk_delete}
                    </button>
                    <button 
                      onClick={() => state.toggleSelectAllDns([])} 
                      className="btn btn-xs btn-ghost text-white/60 hover:text-white rounded-lg px-2"
                    >
                      <X className="size-4" />
                    </button>
                  </div>
              ...
              <div className="max-h-120 overflow-auto border border-slate-200 rounded-2xl relative min-h-60 shadow-inner bg-white custom-scrollbar-light">
                 {state.loadStates.dns && !state.loadStates.bulk ? (
                   ...
                   <table className="table table-xs w-full min-w-180 border-collapse text-left">
                      <thead className="bg-slate-100 text-slate-900 sticky top-0 z-10 shadow-sm"><tr className="font-black text-[10px] uppercase tracking-widest border-b border-slate-200">
                        <th className="px-5 py-4 w-12">
                          <button onClick={() => state.toggleSelectAllDns(filteredDns)} className="flex items-center justify-center hover:text-indigo-600 transition-colors">
                            {state.selectedDnsIds.size === filteredDns.length && filteredDns.length > 0 ? <CheckSquare className="size-4" /> : <Square className="size-4" />}
                          </button>
                        </th>
                        <th className="px-5 py-4 w-20">Type</th><th className="px-5 py-4 w-32">Name</th><th className="px-5 py-4">Content</th><th className="px-5 py-4 text-center w-20">{t.proxy}</th><th className="px-5 py-4 text-right w-32">Action</th></tr></thead>
                      <tbody>
                        {filteredDns.map((r: any) => (
                          <tr key={r.id} className={`hover:bg-indigo-50/50 border-b border-slate-100 last:border-0 transition-colors ${state.editingRecordId === r.id ? 'bg-amber-50/30' : ''} ${state.selectedDnsIds.has(r.id) ? 'bg-indigo-50' : ''}`}>
                            <td className="px-5 py-4">
                              <button onClick={() => state.toggleSelectDns(r.id)} className={`transition-colors ${state.selectedDnsIds.has(r.id) ? 'text-indigo-600' : 'text-slate-300'}`}>
                                {state.selectedDnsIds.has(r.id) ? <CheckSquare className="size-4" /> : <Square className="size-4" />}
                              </button>
                            </td>
                            {state.editingRecordId === r.id ? (
                              <>
                                <td className="px-2 py-2"><select value={state.editFormData.type} onChange={e => state.setEditFormData({...state.editFormData, type: e.target.value, proxied: canBeProxied(e.target.value) ? state.editFormData.proxied : false})} className="select select-xs select-bordered w-full bg-white text-slate-900 font-bold h-9 text-[10px] rounded-lg shadow-sm focus:outline-none">{DNS_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></td>
                                <td className="px-2 py-2"><input type="text" value={state.editFormData.name} onChange={e => state.setEditFormData({...state.editFormData, name: e.target.value})} className="input input-xs input-bordered w-full bg-white text-slate-900 font-bold h-9 text-[10px] rounded-lg shadow-sm" /></td>
                                <td className="px-2 py-2"><input type="text" value={state.editFormData.content} onChange={e => state.setEditFormData({...state.editFormData, content: e.target.value})} className="input input-xs input-bordered w-full bg-white text-slate-900 font-bold h-9 text-[10px] rounded-lg shadow-sm" /></td>
                                <td className="px-2 py-2 text-center">{canBeProxied(state.editFormData.type) ? <button onClick={() => state.setEditFormData({...state.editFormData, proxied: !state.editFormData.proxied})} className={`size-6 mx-auto rounded-full border-2 border-white shadow-sm transition-all flex items-center justify-center ${state.editFormData.proxied ? 'bg-orange-400' : 'bg-slate-200'}`}><div className={`size-1.5 rounded-full bg-white transition-transform ${state.editFormData.proxied ? 'scale-110' : 'scale-75 opacity-40'}`} /></button> : <span className="text-[8px] font-black text-slate-300 uppercase">N/A</span>}</td>
                                <td className="px-2 py-2 text-right"><div className="flex items-center justify-end gap-1.5 pt-1"><button onClick={state.handleSaveEdit} className="btn btn-xs bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm text-white px-2 border-none"><Check className="size-3.5" /></button><button onClick={() => state.setEditingRecordId(null)} className="btn btn-xs btn-ghost bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-600 px-2"><X className="size-3.5" /></button></div></td>
                              </>
                            ) : (
                              <>
                                <td className="px-5 py-4 font-black text-indigo-700 text-[11px]">{r.type}</td>
                                <td className="px-5 py-4 font-bold text-slate-900 text-[11px] truncate max-w-40">{r.name}</td>
                        ...
                        {filteredDns.length === 0 && !state.loadStates.dns && (<tr><td colSpan={6} className="py-20 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.registry_empty}</td></tr>)}
                      </tbody>
                   </table>
                 )}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-8">
               <div className="bg-slate-900 rounded-2xl p-7 shadow-2xl shadow-slate-900/40 space-y-6 flex flex-col min-h-55 relative overflow-hidden text-white">
                  ...
                  {state.loadStates.cert ? <div className="flex-1 flex flex-col items-center justify-center gap-3"><span className="loading loading-spinner loading-lg text-indigo-500"></span></div> : <>
                      <div className="form-control flex-1"><label className="label py-1.5"><span className="label-text text-slate-400 text-[10px] font-black uppercase tracking-widest">{t.authority}</span></label><select value={state.caProvider} onChange={(e) => state.setCaProvider(e.target.value)} className="select select-sm bg-slate-800 text-white border-slate-700 w-full rounded-xl font-bold h-11 focus:ring-2 focus:ring-indigo-500/40 outline-none shadow-inner"><option value="google">Google Trust</option><option value="lets_encrypt">Let's Encrypt</option><option value="ssl_com">SSL.com</option><option value="digicert">DigiCert</option></select></div>
               ...
               <div className="bg-slate-900 rounded-2xl p-7 shadow-2xl shadow-slate-900/40 space-y-6 flex flex-col min-h-55 relative overflow-hidden text-white">
                  ...
                  {state.loadStates.cert ? <div className="flex-1 flex flex-col items-center justify-center gap-3"><span className="loading loading-spinner loading-lg text-indigo-500"></span></div> : <>
                      <div className="form-control flex-1"><label className="label py-1.5"><span className="label-text text-slate-400 text-[10px] font-black uppercase tracking-widest">{t.sec_level}</span></label><select value={state.sslMode} onChange={(e) => state.setSslMode(e.target.value)} className="select select-sm bg-slate-800 text-white border-slate-700 w-full rounded-xl font-bold h-11 focus:ring-2 focus:ring-indigo-500/40 outline-none shadow-inner"><option value="off">Off (Dev Only)</option><option value="flexible">Flexible</option><option value="full">Full</option><option value="strict">Full (Strict)</option></select></div>
                      <button onClick={state.handleApplySSL} disabled={state.loadStates.ssl} className="btn btn-primary btn-md w-full rounded-xl font-black uppercase text-[10px] h-12 text-white hover:bg-indigo-700">{t.enforce_enc}</button>
                    </>}
               </div>
            </div>
          </>
        ) : (
          <div className="h-full min-h-120 bg-white border border-slate-200 border-dashed rounded-3xl flex flex-col items-center justify-center p-16 text-center opacity-50 shadow-inner"><CloudCog className="size-12 text-slate-300 mb-4 animate-bounce" /><h3 className="text-[11px] font-black uppercase tracking-widest text-slate-500">{t.init_context}</h3></div>
        )}
      </div>
    </div>
  );
