...
import { 
  ...
  BookOpen, Layers, Info, Lock, Scale, Menu, X 
} from 'lucide-react';
      ...
      <div className="px-4 py-2">
           ...
           <nav className="space-y-1">
            {[
              ...
              <button 
                key={tab.id} onClick={() => { state.setActiveTab(tab.id as any); state.setIsSidebarOpen(false); }}
      ...
      <aside className={`
       ...
       shadow-2xl lg:shadow-none`}>
        <div className="p-6 flex items-center justify-between">
          ...
          <button onClick={() => state.setIsSidebarOpen(false)} className="lg:hidden text-slate-400 p-2">
            <X className="size-5" />
          </button>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-8 shrink-0 shadow-sm">
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl border text-[9px] sm:text-[10px] font-black uppercase transition-all shadow-sm ${state.isConsoleOpen ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-400 hover:text-slate-900 active:bg-slate-50'}`}
          >
            <Terminal className="size-3.5" /> 
            <span className="hidden xs:inline">{state.isConsoleOpen ? t.monitor_close : t.monitor_open}</span>
          </button>
        </header>
